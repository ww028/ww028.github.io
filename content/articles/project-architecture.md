---
title: 本站技术架构与设计思路
summary: 详细介绍本博客所采用的技术方案、架构设计和关键决策背后的思考。
date: 2026-06-22
pinned: true
...
| 技术                        | 用途                               |
| --------------------------- | ---------------------------------- |
| Next.js 16 (App Router)     | 框架，静态导出生成纯 HTML 站点     |
| React 19                    | UI 组件库                          |
| TypeScript                  | 类型安全                           |
...
- 首页文章列表提供标签筛选按钮，点击可按标签过滤
- 文章详情页显示标签，点击可跳转到首页并自动筛选对应标签
- 标签按使用次数排序显示
...
- 基于 GitHub Discussions，读者用 GitHub 账号即可评论
- 评论数据存在仓库的 Discussions 里，无第三方服务依赖
- 支持表情反应、回复、@提及
- 自动跟随站点明暗主题切换
- 按文章路径自动绑定对应的 Discussion 帖子

### 阅读量统计（不蒜子）

- 基于不蒜子（busuanzi）免费统计服务
- 文章标题下方显示单篇文章阅读次数
- 纯静态站点友好，无需自建后端
- 一行脚本接入，零配置
...
- **JSON-LD**：首页输出 `Blog` 结构化数据，文章页输出 `BlogPosting`
...
- **渐变点缀色**：紫色 → 玫红 → 橙色（`#6c63ff` → `#e91e8c` → `#ff6b35`），仅用于标题文字和按钮
