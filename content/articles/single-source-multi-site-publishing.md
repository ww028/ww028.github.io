...
> 一篇文章，只维护一次；两个网站，各自完整呈现；内容自动流动，但发布权始终掌握在人手里。
...
文章同步可以自动化，但不应该直接越过审查。

更稳妥的链路是：源站文章进入主分支后，由持续集成任务完成同步，并向聚合站创建或刷新一个专用分支上的 Pull Request。
...
因为“文章内容可以自动流动”和“生产环境必须自动发布”是两件不同的事。PR 提供了清晰的变化记录、自动检查结果和最后一次人工确认，同时不迫使自有服务器接入新的部署凭据。
